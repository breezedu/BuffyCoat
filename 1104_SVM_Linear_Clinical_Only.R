...
my_data <- data.frame(clinical.data)
my_data$class <- clinical.data$Progressed
my_data$Progressed <- NULL
...
tiff("Plot_svmLinear_ClinicalOnly.tif", width = 8, height = 8, units = 'in', res = 300)

rocobj_svmlinear <- roc(model_svmLinear$pred$obs, model_svmLinear$pred$yes, ci=TRUE,
                        plot=TRUE, 
                        legacy.axes=TRUE, percent=TRUE,
                        main="Clinical Only Model svmLinear",
                        xlab="False Positive Percentage", 
                        ylab="True Postive Percentage", 
                        col="darkblue", lwd=5, 
                        print.auc=TRUE,
