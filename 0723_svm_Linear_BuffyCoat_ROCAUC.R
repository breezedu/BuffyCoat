...
## run below sorting command only when merging expression with clinical data
...
set.seed(123)
testRowNumbers <- createDataPartition(my_data$class, p=0.77, list=FALSE)
...
num_col <- dim(trainData)[2]
# Store X and Y for later use.
x = trainData[, 1:num_col]
y = trainData$class
y

x_test = testData[ , 1:num_col]
...
lmProfile <- rfe(x=trainData[, 1:num_col-1], 
                 y=trainData$class,
...
## check linear model profile results
lmProfile 
...
plot(varimp_svmLinear, main="Variable Importance with svmLinear Buffy Coat")
varimp_svmLinear$importance
...
rocobj_svmlinear.valid <- roc(model_svmLinear.valid$pred$obs, 
                              model_svmLinear.valid$pred$yes, ci=TRUE,
                              plot=TRUE, 
                              legacy.axes=TRUE, percent=TRUE,
                              main="svmLinear",
                              xlab="False Positive Percentage", 
...
path <- "C:/Users/dug/OneDrive - QIAGEN GmbH/SVM_RFE_Prj/0604/Plots/"
path <- paste0(getwd(), "/Plots/" )
path

PlotROCSaveJPG(rocobj_svmlinear, path, "svmLinear_tune5_train80", "svmLinear" )
...
rocobj_models <- roc(models$rf$pred$obs, 
                     ...
                     plot=TRUE, 
                     legacy.axes=TRUE, percent=TRUE, 
                     main = "Expression Data Multi Model ROCs",
                     xlab="False Positive Percentage", 
                     ylab="True Postive Percentage", 
                     col="darkblue", lwd=4, 
                     print.auc=TRUE,
...
rocobj_models_svmLinear <- roc(models$svmLinear$pred$obs, 
                               ...
                               plot=TRUE, 
                               legacy.axes=TRUE, percent=TRUE, 
                               main = "svm-Linear ROC-AUC Buffy Coat",
                               xlab=" 1 - Specificity", 
                               ylab="Sensitivity", 
                               col="darkblue", lwd=4, 
                               print.auc=TRUE,
...
rocobj_models_svmRadial <- roc(models$svmRadial$pred$obs, 
                               ...
                               plot=TRUE, 
                               legacy.axes=TRUE, percent=TRUE, 
                               main = "Expression ROCs svm-Radial",
                               xlab="False Positive Percentage", 
                               ylab="True Postive Percentage", 
                               col="darkblue", lwd=4, 
                               print.auc=TRUE,
...
rocobj_models_svmRadialVa <- roc(valid.models$svmRadial$pred$obs, 
...
rocobj_models_rf <- roc(models$rf$pred$obs, 
                        models$rf$pred$yes, 
                        ci=TRUE,
                        plot=TRUE, 
                        legacy.axes=TRUE, percent=TRUE, 
                        main = "Expression ROCs Random Forest",
                        xlab="False Positive Percentage", 
                        ylab="True Postive Percentage", 
                        col="darkblue", lwd=4, 
                        print.auc=TRUE,
...
rocobj_models_rfVa <- roc(valid.models$rf$pred$obs, 
...
rocobj_models_knn <- roc(models$knn$pred$obs, 
                         models$knn$pred$yes, 
                         ci=TRUE,
                         plot=TRUE, 
                         legacy.axes=TRUE, percent=TRUE, 
                         main = "Expression ROCs KNN",
                         xlab="False Positive Percentage", 
                         ylab="True Postive Percentage", 
                         col="darkblue", lwd=4, 
                         print.auc=TRUE,
...
rocobj_models_knnva <- roc(valid.models$knn$pred$obs, 
...
rocobj_models_xgbDART <- roc(models$xgbDART$pred$obs, 
                             models$xgbDART$pred$yes, 
                             ci=TRUE,
                             plot=TRUE, 
                             legacy.axes=TRUE, percent=TRUE, 
                             main = "Expression ROCs xgbDART",
                             xlab="False Positive Percentage", 
                             ylab="True Postive Percentage", 
                             col="darkblue", lwd=4, 
                             print.auc=TRUE,
...
rocobj_models_xgbDARTVa <- roc(valid.models$xgbDART$pred$obs, 
...
rocobj_models_marsVa <- roc(valid.models$earth$pred$obs, 
