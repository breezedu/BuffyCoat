...
setwd("C:/Users/Jeff/OneDrive - QIAGEN GmbH/SVM_RFE_Prj/Elain_final_reports/")

getwd()

count <- read.table("./tissue_samples/Count.txt", row.names = 1, header = T, sep = "\t")
design <- read.table("./tissue_samples/Count_Design.txt", row.names = 1, header = T, sep = "\t")
top.genes <- readLines("./tissue_samples/Deseq2_top45rows.txt")
...
#########
### get BuffyCoat expression data; 
## setwd("C:/Users/Jeff/OneDrive - QIAGEN GmbH/SVM_RFE_Prj/0604")

count.bc <- read.table("./buffycoat_samples/Counts.Data.txt", row.names = 1, header = T, sep = "\t")
design.bc <- read.table("./buffycoat_samples/Counts.Data_Design.txt", row.names = 1, header = T, sep = "\t")
top.genes.bc <- readLines("./buffycoat_samples/Deseq2_top47row.txt")

clinical.data <- read.table("./buffycoat_samples/Clinical_Data.txt", row.names = 1, header = T, sep = "\t")
...
top.genes.all <- c( top.genes, top.genes.bc)
top.genes.all
unique(top.genes.all)
...
datamatrix <- count
datamatrix.bc <- count.bc

dim(datamatrix)
dim(datamatrix.bc)

constantRow <- row.names(as.matrix(which(apply(datamatrix, MARGIN = 1, function(x) var(x) < 0.001) ) ) ) 
constantRow.bc <- row.names(as.matrix(which(apply(datamatrix.bc, MARGIN = 1, function(x) var(x) < 0.001) ) ) ) 
...
#exclude rows with constant value: 
count.bc <- datamatrix.bc[!row.names(datamatrix.bc) %in% constantRow.bc,] 

count.bc <- datamatrix.bc[ row.names(datamatrix.bc) %in% top.genes.bc, ]
...
my_data.bc <- as.data.frame( t(count.bc))
...
#### 
##  col bind my_data and my_data.bc by row names (sample ids)

my_data.all <- merge(my_data, my_data.bc, by='row.names', all = T)

#my_data.all <- cbind(my_data, my_data.bc[, "symbol"][match(rownames(my_data), rownames(my_data.bc))])

dim(my_data.all)

head(my_data.all)

rownames(my_data.all) <- my_data.all$Row.names
my_data.all$Row.names <- NULL 


##########################################
## 
## Update data frame on 7/25/2019, 
## is it ok to merge two panCancer expression data into one, by bruceforce method?
...
mydata.clinical <- merge(my_data.all, as.data.frame(clinical.data), by='row.names', all=TRUE)
...
rownames(mydata.clinical) <- mydata.clinical$Row.names
mydata.clinical$Row.names <- NULL
...
set.seed(100)
trainRowNumbers <- createDataPartition(my_data$class, p=0.75, list=FALSE)

#set.seed(123)
#testRowNumbers <- createDataPartition(my_data$class, p=0.78, list=FALSE)
...
model_svmLinear$pred$obs
model_svmLinear$pred$yes


predicted2 <- predict(model_svmLinear, testData, type = "prob")

str( predicted2 )

rocobj_svmlinear.vliad <- roc( testData$class, predicted2[[2]] , ci=TRUE,
                              plot=TRUE, 
                              legacy.axes=TRUE, percent=TRUE,
                              main="svmLinear",
                              xlab="False Positive Percentage", 
...
multiclass.roc(testData$class, predicted2)
...
setwd("D:/miRNA_Prj/0526_MergedDataPlots/")
path <- getwd()
path
...
model_xgbDART.valid = train(class ~ ., 
                            data=testData, 
                            method='xgbDART', 
                            tuneLength=4, 
                            trControl = fitControl, 
...
rocobj_xgbDART.valid <- roc(model_xgbDART.valid$pred$obs, 
                            model_xgbDART.valid$pred$yes, 
                            ci=TRUE,
                            plot=TRUE, 
                            legacy.axes=TRUE, percent=TRUE, 
                            main = "xgbDART Combined ROC",
